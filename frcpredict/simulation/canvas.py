...
def expand_kernels_to_2d(*kernels, pixels_per_nm: float) -> np.ndarray:
    ...
    canvas_outer_rad_px, _ = get_canvas_dimensions_px(
        get_canvas_radius_nm(extend_sides_to_diagonal=True), pixels_per_nm
    )
...
def expand_dimensions(radialvec, radialval, outradius):  # TODO: Different from _canvas_meshgrid?
