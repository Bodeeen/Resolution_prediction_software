...
class SampleStructure:
    ...
    image: SampleImage = field(default_factory=SampleImage)
    ...
    def from_sample_data_directory_name(cls, dir_name: str) -> "SampleStructure":
        ...
        image = SampleImage(
            id=dir_name, image_arr=Array2DPatternData.from_image_file(image_file_path).value
        )

        return cls(properties=properties, image=image)
