...
def spinavej(x):
    ...
    """
    if nr >= 512:
        sumf = np.zeros(int(maxindex))
        count = np.zeros(int(maxindex))
        for ri in range(nr):
            for ci in range(nc):
