...
from frcpredict.util import make_lines_structure


class LinesStructureDialog(QDialog, BaseWidget):
    """
    A dialog for generating a lines sample structure.
    """
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.editNumLines.valueChanged.connect(self._updateOKButton)
    ...
    def getStructure(parent: Optional[QWidget] = None) -> Tuple[Optional[SampleStructure], bool]:
        ...
        dialog = LinesStructureDialog(parent)
        result = dialog.exec_()

        if result == QDialog.Accepted:
            areaSide = dialog.editAreaSide.value()

            relativeArrayX, relativeArrayY = make_lines_structure(
                area_side_um=areaSide,
                num_lines=dialog.editNumLines.value(),
                f_per_um=dialog.editFluorophores.value()
            )
